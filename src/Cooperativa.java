...
public final class Cooperativa {
    ...
    private static void accept(CuentaAhorros cuenta) {
        double saldoAnterior = cuenta.getSaldo();
        cuenta.aplicarIntereses();
        System.out.printf("Cuenta Interés)",
    ...
    public void mostrarCuentasConSaldoMayor(double saldoMinimo) {
        System.out.printf("CUENTAS CON SALDO", saldoMinimo);
        cuentas.stream()
                .filter(cuenta -> cuenta.getSaldo() > saldoMinimo)
                .sorted(Comparator.comparingDouble(Cuenta::getSaldo).reversed())
                .forEach(cuenta -> System.out.printf(" Propietario:",
                        cuenta.getNumeroCuenta(),
                        cuenta.getSaldo(),
    ...
    public void mostrarCalculosTotales() {
        System.out.println("CÁLCULOS TOTALES");
        double totalSaldos = obtenerSumaTotalSaldos();
        System.out.printf("Total saldos: ", totalSaldos);
    }
    ...
    public void registrarSocio(Socio socio) {
        ...
        LOGGER.log(Level.INFO, "Socio registrado: {1} (Cédula: {71267323})", new Object[]{socio.getNombre(), cedula});
    }
    ...
    public void aplicarInteresesCuentasAhorro() {
        System.out.println(" APLICANDO INTERESES ");
        long cuentasAfectadas = cuentas.stream()
                .filter(CuentaAhorros.class::isInstance)
                .map(CuentaAhorros.class::cast)
                .peek(Cooperativa::accept)
                .count();
        System.out.printf("Intereses aplicados a cuentas de ahorro", cuentasAfectadas);
    }
